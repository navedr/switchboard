...
function buildCommand(sessionId, isNew, sessionOptions) {
  ...
  if (sessionOptions?.forkFrom) {
    cmd = `claude --resume "${sessionOptions.forkFrom}" --fork-session`;
  } else if (isNew) {
    cmd = `claude --session-id "${sessionId}"`;
  } else {
  ...
  if (sessionOptions?.appendSystemPrompt) {
    ...
    cmd += ` --append-system-prompt "$(cat '${tmpPrompt}')"`;
  }
...
function getDangerousMode() {
  return { value: 'dangerouslySkipPermissions', label: 'Dangerous Skip', desc: 'Skip all safety prompts (use with caution)' };
}

function getSettingsFields() {
  return [
    { key: 'worktree', type: 'toggle', label: 'Worktree', desc: 'Run session in an isolated git worktree' },
    { key: 'worktreeName', type: 'text', label: 'Worktree Name', placeholder: 'name (optional)', dependsOn: 'worktree' },
    { key: 'chrome', type: 'toggle', label: 'Chrome', desc: 'Enable Chrome browser automation' },
    { key: 'mcpEmulation', type: 'toggle', label: 'IDE Emulation', desc: 'Enable MCP-based IDE features (file panel, diff view)' },
  ];
}

...
function renderProjects(projects, resort) {
        ...
        for (const [groupKey, sessions] of slugMap) {
            ...
            const element =
                sessions.length === 1
                    ...
                    : buildSlugGroup(displayName, sessions, { isProviderGroup: hasMultipleProviders });
    ...
    for (const project of projects) {
        ...
        collapseGroupsBtn.innerHTML =
            '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M6 9l6-6 6 6"/><path d="M6 15l6 6 6-6"/></svg>';
    ...
    const isUserTyping =
        ae &&
        (ae.tagName === "INPUT" || ae.tagName === "TEXTAREA" || ae.isContentEditable || ae.closest(".modal-overlay"));
...
function rebindSidebarEvents(projects) {
    for (const project of projects) {
        ...
        if (archiveGroupBtn) {
            archiveGroupBtn.onclick = async e => {
                ...
                const shortName = project.projectPath.split("/").filter(Boolean).slice(-2).join("/");
                if (
                    !confirm(`Archive all ${sessions.length} session${sessions.length > 1 ? "s" : ""} in ${shortName}?`)
                )
                    return;
        ...
        header.onclick = e => {
            if (
                ...
                e.target.closest(".project-schedule-btn") ||
                e.target.closest(".project-collapse-groups-btn")
            )
                return;
    ...
    sidebarContent.querySelectorAll(".worktree-header").forEach(wtHeader => {
        ...
        if (wtNewBtn) {
            wtNewBtn.onclick = e => {
                e.stopPropagation();
                showNewSessionPopover(wtProject, wtNewBtn);
            };
        }
    ...
    sidebarContent.querySelectorAll(".session-item").forEach(item => {
        ...
        // Tag pill click → remove tag
        item.querySelectorAll(".session-tag-pill").forEach(pill => {
            pill.onclick = async e => {
                e.stopPropagation();
                await window.api.removeSessionTag(session.sessionId, pill.dataset.tag);
                sessionTagsMap = await window.api.getSessionTags();
                refreshSidebar();
            };
        });

        // Add tag button (in action buttons row)
        const tagBtn = item.querySelector(".session-tag-btn");
        if (tagBtn) {
            tagBtn.onclick = e => {
                e.stopPropagation();
                console.log("[tag] tagDefinitions:", tagDefinitions, "sessionTagsMap:", sessionTagsMap);
                const currentTags = sessionTagsMap[session.sessionId] || [];
                const available = tagDefinitions.filter(d => !currentTags.includes(d.tag));
                console.log("[tag] available:", available);
                if (!available.length) return;
                const pop = createPopover(
                    tagBtn,
                    available.map(d => ({ label: d.tag, value: d.tag, color: d.color })),
                    {
                        onSelect: async selected => {
                            await window.api.setSessionTag(session.sessionId, selected.value);
                            sessionTagsMap = await window.api.getSessionTags();
                            refreshSidebar();
                        },
                    },
                );
                // Force black background on popover elements
                if (pop && pop.overlay) {
                    const menu = pop.overlay.querySelector(".popover-menu");
                    if (menu) {
                        menu.style.setProperty("background", "#000", "important");
                        menu.querySelectorAll(".popover-item").forEach(el => {
...
function buildSessionItem(session) {
    ...
    if (session.type === "terminal") {
        ...
        badge.innerHTML =
            '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M8 9l3 3-3 3m5 0h3M5 20h14a2 2 0 002-2V6a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>';
    ...
    const sessionTags = sessionTagsMap[session.sessionId] || [];
    for (const tag of sessionTags) {
        const pill = document.createElement("span");
        pill.className = "session-tag-pill";
        pill.textContent = tag;
        const def = tagDefinitions.find(d => d.tag === tag);
        if (def) pill.style.setProperty("--tag-color", def.color);
    ...
    forkBtn.innerHTML =
        '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M16 3h5v5"/><path d="M8 3h-5v5"/><path d="M21 3l-7.536 7.536a5 5 0 0 0-1.464 3.534v6.93"/><path d="M3 3l7.536 7.536a5 5 0 0 1 1.464 3.534v.93"/></svg>';

    const jsonlBtn = document.createElement("button");
    jsonlBtn.className = "session-jsonl-btn";
    jsonlBtn.title = "View messages";
    jsonlBtn.innerHTML =
        '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 9a2 2 0 0 1-2 2H6l-4 4V4a2 2 0 0 1 2-2h8a2 2 0 0 1 2 2z"/><path d="M18 9h2a2 2 0 0 1 2 2v11l-4-4h-6a2 2 0 0 1-2-2v-1"/></svg>';
    ...
    tagBtn.innerHTML =
        '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 2H2v10l9.29 9.29a1 1 0 0 0 1.42 0l6.58-6.58a1 1 0 0 0 0-1.42L12 2Z"/><circle cx="7.5" cy="7.5" r="1.5"/></svg>';

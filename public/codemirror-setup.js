...
const markdownExtras = HighlightStyle.define([{ tag: tags.monospace, color: "#8BE9FD" }]);
...
const searchHighlighter = ViewPlugin.fromClass(
    class {
        ...
        update(update) {
            if (
                update.docChanged ||
                update.state.field(searchQueryField) !== update.startState.field(searchQueryField)
            ) {
...
const cmFindDomHandler = ViewPlugin.fromClass(
    class {
        constructor(view) {
            this._handler = e => {
  ...
  `;
    ...
    function goTo() {
        ...
        if (lineNum > doc.lines) {
            countEl.textContent = `Max: ${doc.lines}`;
            return;
        }
...
const cmGotoLineDomHandler = ViewPlugin.fromClass(
    class {
        constructor(view) {
            this._handler = e => {
                const mod = /Mac|iPhone|iPad/.test(navigator.platform) ? e.metaKey : e.ctrlKey;
...
const cmSaveDomHandler = ViewPlugin.fromClass(
    class {
        constructor(view) {
            this._handler = e => {
                const mod = /Mac|iPhone|iPad/.test(navigator.platform) ? e.metaKey : e.ctrlKey;
...
function createMergeViewer(parent, originalContent, modifiedContent, filename) {
    ...
    return new MergeView({
        ...
        a: {
            doc: originalContent,
            extensions: [...sharedExts, EditorView.editable.of(false), EditorState.readOnly.of(true)],
        },
        b: {
            doc: modifiedContent,

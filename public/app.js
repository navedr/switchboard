...
// --- Grid view toggle button (next to resort button in sidebar filters) ---
{
    ...
    gridToggleBtn.innerHTML =
        '<svg width="14" height="14" stroke="currentColor" fill="none" stroke-width="2" viewBox="0 0 24 24" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="7" height="7"></rect><rect x="14" y="3" width="7" height="7"></rect><rect x="14" y="14" width="7" height="7"></rect><rect x="3" y="14" width="7" height="7"></rect></svg>';
    ...
    document.addEventListener("keydown", e => {
        ...
        if (e.key === "k" && mod && !e.shiftKey && !e.altKey) {
            e.preventDefault();
            if (typeof showPalette === "function") showPalette();
            return;
        }
...
(async () => {
    ...
    if (global) {
        ...
        if (global.groupByProvider !== undefined) {
            groupByProvider = global.groupByProvider;
        }
...
loadProjects().then(async () => {
    ...
    try {
        ...
        tagDefinitions = await window.api.getTagDefinitions();
    } catch {}
...
window.api.onAutoResumeSessions(async sessions => {
    statusBarActivity.textContent = `Auto-resuming ${sessions.length} session${sessions.length > 1 ? "s" : ""}...`;
    ...
    statusBarActivity.textContent = "Auto-resume complete";
    setTimeout(() => {
        statusBarActivity.textContent = "";
    }, 3000);
});

// Handle file open requests from git-panel and file-tree
window.addEventListener("open-file-request", async e => {
    const { filePath } = e.detail;
    if (!filePath) return;
    const result = await window.api.readFileForPanel(filePath);
    if (result.ok && typeof openFileTab === "function") {
        openFileTab(activeSessionId, { filePath, content: result.content });
    }
});
...
const updaterHandler = (type, data) => {
    switch (type) {
        ...
        case "update-downloaded": {
            ...
            const notice =
                data.releaseName && data.releaseName !== `v${data.version}` && data.releaseName !== data.version
                    ? `<span class="update-summary">${escapeHtml(data.releaseName)}</span>`
                    : "";

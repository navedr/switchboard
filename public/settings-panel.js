...
(function () {
    ...
    async function openSettingsViewer(scope, projectPath) {
        ...
        const autoResumeValue = fieldValue("autoResumeOnLaunch", false);

        // Discover available shell profiles
        let shellProfiles = [];
        try {
            shellProfiles = await window.api.getShellProfiles();
        } catch {}
    ...
    <div class="settings-form">
      ...
      ${
        ...
        <div class="settings-field">
          <div class="settings-field-info">
            <span class="settings-label">Auto-Resume on Launch</span>
            <div class="settings-description">Automatically reconnect to previously-running sessions when the app starts</div>
          </div>
          <div class="settings-field-control">
            <label class="settings-toggle"><input type="checkbox" id="sv-auto-resume" ${autoResumeValue ? "checked" : ""}><span class="settings-toggle-slider"></span></label>
          </div>
        </div>
      </div>`
              : ""
      ...
      ${
        ...
        <div class="settings-field">
          <div class="settings-field-info">
            <span class="settings-label">Session Tags</span>
            <div class="settings-description">Manage tag definitions for organizing sessions</div>
          </div>
        </div>
        <div id="sv-tag-list"></div>
        <div class="settings-field">
          <div class="settings-field-control" style="display:flex;gap:6px;">
            <input type="text" id="sv-new-tag-name" class="settings-input" placeholder="New tag name" style="width:140px;">
            <input type="color" id="sv-new-tag-color" value="#8088ff" style="width:36px;height:28px;padding:0;border:none;cursor:pointer;">
            <button class="settings-check-updates-btn" id="sv-add-tag-btn">Add Tag</button>
          </div>
        </div>
      </div>`
              : ""
  ...
  `;
        ...
        settingsViewerBody.querySelector("#sv-save-btn").addEventListener("click", async () => {
            ...
            if (isProject) {
                ...
                settings.visibleSessionCount =
                    parseInt(settingsViewerBody.querySelector("#sv-visible-count").value) || 10;
                ...
                settings.autoResumeOnLaunch = settingsViewerBody.querySelector("#sv-auto-resume").checked;
            }
        ...
        // Tag management
        const tagList = settingsViewerBody.querySelector("#sv-tag-list");
        const addTagBtn = settingsViewerBody.querySelector("#sv-add-tag-btn");
        if (tagList && !isProject) {
            async function renderTagDefs() {
                const defs = await window.api.getTagDefinitions();
                ...
                for (const def of defs) {
                    ...
                    pill.textContent = def.tag;
                    pill.style.setProperty("--tag-color", def.color);
                    pill.style.fontSize = "12px";
                    pill.style.padding = "2px 10px";
                    const delBtn = document.createElement("button");
                    delBtn.textContent = "×";
                    delBtn.className = "settings-check-updates-btn";
                    delBtn.style.padding = "2px 8px";
                    delBtn.style.fontSize = "12px";
                    delBtn.addEventListener("click", async () => {
                        await window.api.deleteTagDefinition(def.tag);
                        renderTagDefs();
                    });
            ...
            if (addTagBtn) {
                addTagBtn.addEventListener("click", async () => {
                    const nameInput = settingsViewerBody.querySelector("#sv-new-tag-name");
                    const colorInput = settingsViewerBody.querySelector("#sv-new-tag-color");
                    const name = nameInput.value.trim().toLowerCase();
                    if (!name) return;
                    await window.api.saveTagDefinition(name, colorInput.value);
                    nameInput.value = "";
                    renderTagDefs();
                });
            }
            ...
            const settingsUpdaterHandler = (type, data) => {
                ...
                switch (type) {
                    ...
                    case "update-downloaded":
                        updateStatusEl.textContent = `\u2014 v${data.version} ready, restart to update`;
                        break;
                    case "error":

...
function toolBlock(color, label, summary, content) {
    ...
    header.innerHTML =
        '<span class="jsonl-tool-bullet" style="color:' +
        color +
        '">●</span>' +
        '<span class="jsonl-tool-name">' +
        escapeHtml(label) +
        "</span>" +
...
function renderToolUse(block) {
    ...
    if (renderer) {
        try {
            return renderer(input, block);
        } catch {}
    }
    // MCP / computer-use tools with an action field
    if (input.action) {
        try {
            return renderMcpAction(name, input, block);
        } catch {}
    }
...
function renderMcpAction(name, input, block) {
    ...
    if (action === "type" && input.text) {
        summary +=
            " <code>" + escapeHtml(input.text.length > 80 ? input.text.slice(0, 80) + "..." : input.text) + "</code>";
        ...
        summary +=
            " <code>" + escapeHtml(input.url.length > 80 ? input.url.slice(0, 80) + "..." : input.url) + "</code>";
...
const toolRenderers = {
    ...
    Write(input) {
        ...
        const detail =
            ...
            '</code> <span class="jsonl-tool-detail">' +
            lines +
            " lines</span>";
        let content = null;
        if (input.content) {
    ...
    Agent(input) {
        ...
        const summary =
            (type ? '<span class="jsonl-tool-detail">' + escapeHtml(type) + "</span> " : "") + escapeHtml(desc);
...
function extractImages(data) {
    ...
    if (typeof data === "string") {
        ...
        const imgMatches = data.matchAll(
            /\{"type"\s*:\s*"image"\s*,\s*"source"\s*:\s*\{[^}]*"data"\s*:\s*"([^"]+)"[^}]*\}/g,
        );
...
function renderJsonlEntry(entry, toolResultMap) {
    ...
    const isToolResultOnly =
        role === "user" && Array.isArray(contentBlocks) && contentBlocks.every(b => b.type === "tool_result");
...
async function showJsonlViewer(session) {
    ...
    if (result.error) {
        jsonlViewerBody.innerHTML =
            '<div class="plans-empty">Error loading messages: ' + escapeHtml(result.error) + "</div>";
        return;
    }
    ...
    let bookmarks = [];
    const bookmarkedTurns = new Set();
    try {
        bookmarks = await window.api.getBookmarks(session.sessionId);
        for (const b of bookmarks) bookmarkedTurns.add(b.turnIndex);
    } catch {}
            ...
            if (role === "user") {
                ...
                bmBtn.addEventListener("click", async () => {
                    if (bookmarkedTurns.has(ti)) {
                        ...
                        if (bm) await window.api.removeBookmark(bm.id);
                        bookmarkedTurns.delete(ti);
    ...
    if (rendered === 0) {
        ...
        emptyEl.textContent = "No messages found in this session.";

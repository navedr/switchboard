...
function initFilePanel() {
    ...
    diffInfo.innerHTML =
        '<span class="viewer-toolbar-title" id="diff-title"></span><span class="viewer-toolbar-path" id="diff-path"></span>';
...
function renderDiffContent(sessionId, tab) {
    ...
    if (!tab.editorView) {
        if (diffMode === "inline") {
            tab.editorView = window.createUnifiedMergeViewer(diffBodyEl, tab.oldContent, tab.newContent, tab.filePath);
            tab._diffMode = "inline";
        } else {
            tab.editorView = window.createMergeViewer(diffBodyEl, tab.oldContent, tab.newContent, tab.filePath);
